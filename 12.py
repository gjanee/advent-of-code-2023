...
# you were looking for the hot springs, weren't you?"  You indicate
...
#
# "Hello!  What brings you to the hot springs today?  Sorry they're
# not very hot right now; we're having a lava shortage at the moment."
...
# safe to use!  Worse yet, their condition records of which springs
# are damaged (your puzzle input) are also damaged!  You'll need to
# help them repair the damaged records.
#
...
# is operational (.) or damaged (#).  This is the part of the
...
# duplicated some of this information in a different format!  After
# the list of springs for a given row, the size of each contiguous
# group of damaged springs is listed in the order those groups appear
# in the row.  This list always accounts for every damaged spring, and
# each number is the entire size of its contiguous group (that is,
...
# broken (to satisfy the final contiguous group of three broken
# springs), and each ?? must hide exactly one of the two broken
# springs.  (Neither ?? could be both broken springs or they would
# form a single contiguous group of two; if that were true, the
# numbers afterward would have been 2,3 instead.)  Since each ?? can
# either be #. or .#, there are four possible arrangements of springs.
#
# The last line is actually consistent with ten different
# arrangements!  Because the first number is 3, the first and second ?
# must both be . (if either were #, the first number would have to be
# 4 or higher).  However, the remaining run of unknown spring
# conditions have many different ways they could hold groups of two
...
# .###....##.#
#
# In this example, the number of possible arrangements for each row
# is:
#
# - ???.### 1,1,3 - 1 arrangement
...
for line in open("12.in"):
    ...
    nums = tuple(int(v) for v in nums.split(","))
    conditions.append((pattern, nums))

@lru_cache(maxsize=None)
def num_matches(pattern, nums):
    if len(nums) == 0:
        return 1 if re.match("[.?]*$", pattern) else 0
    n = nums[0]
    # We focus on fitting just the first number (and its trailing
    # spacer character).  The remaining numbers will require a number
    # of characters equal to their sum plus an additional trailing
    # spacer character for each, leaving us with `w` characters to
    # work with for the first number.
    if not pattern.endswith("."):
        pattern += "."
    w = len(pattern) - (sum(nums[1:]) + len(nums)-1)
    return sum(
        num_matches(pattern[i+n+1:], nums[1:])
        for i in range(w-n)
        if re.match("[.?]{%d}[#?]{%d}[.?]" % (i, n), pattern)
    )

print(sum(num_matches(pattern, nums) for pattern, nums in conditions))
...
conditions = [
    ("?".join([pattern]*5), nums*5)
    for pattern, nums in conditions
]
