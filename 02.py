...
# It's quite cold, but you don't see much snow.  An Elf runs over to
# greet you.
#
...
Game = namedtuple("Game", "id grabs")

games = []
for line in open("02.in"):
    id, s = re.match(r"Game (\d+): (.*)\n$", line).groups()
    games.append(
        Game(
            int(id),
            [
                Counter(
                    {
                        count.split()[1]: int(count.split()[0])
                        for count in grab.split(", ")
                    }
                )
                for grab in s.split("; ")
            ]
...
print(
    sum(
        ...
        for game in games
        if all(
            g["red"] <= 12 and g["green"] <= 13 and g["blue"] <= 14
            for g in game.grabs
        )
    )
)
...
#   green, and 6 blue cubes.  If any color had even one fewer cube,
...
print(
    sum(
        prod(
            max(g[color] for g in game.grabs)
