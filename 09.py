...
# look up, you discover what must be yet another giant floating
# island, this one made of metal!  That must be where the parts to fix
...
# Each line in the report contains the history of a single value.  For
# example:
#
# 0 3 6 9 12 15
# 1 3 6 10 15 21
...
# step it considers two numbers from the input.  Since these values
# aren't all zero, repeat the process: the values differ by 0 at each
# step, so the next sequence is 0 0 0 0.  This means you have enough
# information to extrapolate the history!  Visually, these sequences
...
#     0   0   0   0
#
# To extrapolate, start by adding a new zero to the end of your list
# of zeroes; because the zeroes represent differences between the two
# values above them, this also means there is now a placeholder in
...
def next_value(sequence):
    ...
    while True:
        ...
        if all(v == 0 for v in b):
            break
        diffs.append(b)
    return sum(s[-1] for s in diffs)

print(sum(next_value(s) for s in sequences))

# --- Part Two ---
#
# Of course, it would be nice to have even more history included in
# your report.  Surely it's safe to just extrapolate backwards as
# well, right?
#
# For each history, repeat the process of finding differences until
# the sequence of differences is entirely zero.  Then, rather than
# adding a zero to the end and filling in the next values of each
# previous sequence, you should instead add a zero to the beginning of
# your sequence of zeroes, then fill in new first values for each
# previous sequence.
#
...
# previous value for each history.  What is the sum of these
# extrapolated values?
#
# --------------------
#
# Haha, just reverse the sequences.  We need to subtract at each step
# instead of add, but by working in reverse all the deltas get
# negated, so we can stick with adding.

print(sum(next_value(s[::-1]) for s in sequences))
