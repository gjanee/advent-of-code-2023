...
# ride in an airship.  (At least it's a cool airship!)  It drops you
# off at the edge of a vast desert and descends back to Island Island.
#
...
# what look like very large rocks covering half of the horizon.  The
# Elf explains that the rocks are all along the part of Desert Island
# that is directly above Island Island, making it hard to even get
...
#
# Because the journey will take a few days, she offers to teach you
# the game of Camel Cards.  Camel Cards is sort of similar to poker
...
# cards labeled one of A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, or 2.  The
# relative strength of each card follows this order, where A is the
# highest and 2 is the lowest.
#
# Every hand is exactly one type.  From strongest to weakest, they
# are:
#
...
# effect.  Start by comparing the first card in each hand.  If these
# cards are different, the hand with the stronger first card is
# considered stronger.  If the first card in each hand have the same
# label, however, then move on to considering the second card in each
# hand.  If they differ, the hand with the higher second card wins;
# otherwise, continue with the third card in each hand, then the
...
# card is stronger (and both hands have the same first and second
# card).
#
...
# gets rank 2, and so on up to the strongest hand.  Because there are
# five hands in this example, the strongest hand will have rank 5 and
...
#   the same label, but the second card of KK677 is stronger (K vs T),
...
#
# Find the rank of every hand in your set.  What are the total
# winnings?
...
def type(hand):
    # Return a hand's type as a list of card repetition counts in
    # descending order, e.g., a full house is [3, 2].
...
def solve():
    ...
    for line in open("07.in"):
        hand, bid = line.split()
        handbids.append((type(hand), rank(hand), int(bid)))
    handbids.sort()
    return sum(hb[2]*i for i, hb in enumerate(handbids, start=1))
...
# additional rule.  Now, J cards are jokers - wildcards that can act
...
# a kind.  However, for the purpose of breaking ties between two hands
# of the same type, J is always treated as J, not the card it's
...
# --------------------
#
# Hand strength is always maximized by turning jokers into the
# most-frequently occurring card.
