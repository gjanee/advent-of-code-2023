...
# massive parabolic reflector dish attached to the side of another
# large mountain.
#
...
# direction.  If the dish is meant to focus light, all it's doing
# right now is sending it in a vague direction.
#
# This system must be what provides the energy for the lava!  If you
# focus the reflector dish, maybe you can go where it's pointing and
# use the light to fix the lava production.
#
...
# metal platform below the dish.  The platform is covered in large
# rocks of various shapes.  Depending on their position, the weight of
# the rocks deforms the platform, and the shape of the platform
# controls which ropes move and ultimately the focus of the dish.
#
# In short: if you move the rocks, you can focus the dish.  The
# platform even has a control panel on the side that lets you tilt it
# in one of four directions!  The rounded rocks (O) will roll when the
# platform is tilted, while the cube-shaped rocks (#) will stay in
# place.  You note the positions of all of the empty spaces (.) and
...
# #....#....
#
# You notice that the support beams along the north side of the
# platform are damaged; to ensure the platform doesn't collapse, you
# should calculate the total load on the north support beams.
#
...
def load_grid():
    # Return (square) grid and common side length.
    grid = {
        (r, c): v
        for r, line in enumerate(open("14.in"))
...
def tilt(direction="N"):
    # Tilt the platform, modifying `grid`.  The code below is written
    # to always tilt to the north; tilting in other directions is
    # achieved by using a coordinate transformation.
    coord_xforms = {
        "N": lambda r, c: (r, c),
        "W": lambda r, c: (c, r),
        "S": lambda r, c: (D-r-1, c),
        "E": lambda r, c: (c, D-r-1)
    }
    m = coord_xforms[direction]
    for r in range(D):
        for c in range(D):
            if grid[m(r, c)] == "O":
                nr = r-1
                while nr >= 0 and grid[m(nr, c)] == ".":
                    nr -= 1
                # At this point, position (nr, c) is either off the
                # platform or the position of a block.
                grid[m(r, c)] = "."
                grid[m(nr+1, c)] = "O"

def north_beam_load():
    return sum(
        sum(grid[(r, c)] == "O" for c in range(D)) * (D-r)
        for r in range(D)
    )
...
# the platform.  Fortunately, a button on the side of the control
...
# rounded rocks roll as far as they can before the platform tilts in
# the next direction.  After one cycle, the platform will have
# finished rolling the rounded rocks in those four directions in that
# order.
#
...
grid = path[(1000000000-i)%cycle_len+i]
