...
# find that the light is being collected by the wall of a large
...
# panicked reindeer wearing goggles and a loose-fitting hard hat.  The
# reindeer leads you to a shelf of goggles and hard hats (you quickly
# find some that fit) and then further into the facility.  At one
# point, you pass a button with a faint snout mark and the label "PUSH
# FOR HELP".  No wonder you were loaded into that trebuchet so
# quickly!
#
# You pass through a final set of doors surrounded with even more
# warning signs and into what must be the room that collects all of
# the light from outside.  As you admire the large assortment of
# lenses available to further focus the light, the reindeer brings you
...
# string, start with a current value of 0.  Then, for each character
...
#
# After following these steps for each character in the string in
...
# your HASH algorithm is working, the book offers the sum of the
# result of running the HASH algorithm on each step in the
...
# result of running the HASH algorithm on each of the steps is as
# follows:
#
# - rn=1 becomes 30.
# - cm- becomes 253.
...
# the reindeer has stolen the page containing the expected
# verification number and is currently running around the facility
# with it excitedly.
#
...
steps = open("15.in").read().strip().split(",")
...
# through 255.  The boxes are arranged in a line starting from the
# point where light enters the facility.  The boxes have holes that
# allow light to pass from one box to the next all the way down the
# line.
#
#       +-----+  +-----+         +-----+
# Light | Box |  | Box |   ...   | Box |
...
# correctly positioned to focus light passing through the box.  The
# side of each box has a panel that opens to allow you to insert or
...
# 9.  The reindeer also brings you a small handheld label printer.
#
...
# mark the lens with the label given in the beginning of the step so
...
#   box.  Don't move any of the other lenses when you do this.  If
...
# lenses are shown here.  Within each box, lenses are listed from
# front to back; each lens is shown as its label and focal length in
# square brackets.
#
# To confirm that all of the lenses are installed correctly, add up
# the focusing power of all of the lenses.  The focusing power of a
...
#
# At the end of the above example, the focusing power of each lens is
# as follows:
#
...
for s in steps:
    ...
    i = next(filter(lambda i: lenses[i][0] == label, range(len(lenses))), None)
    if op == "add":
        if i != None:
            lenses[i] = (label, fp)
        else:
            lenses.append((label, fp))
    else:
        if i != None:
            del lenses[i]
...
print(
    sum(
        sum(
            i*j*fp
            for j, (label, fp) in enumerate(lenses, start=1)
        )
