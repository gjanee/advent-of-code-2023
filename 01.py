...
# snow comes from") when you realize that the Elves are already
...
# be found by combining the first digit and the last digit (in that
...
def solve():
    return sum(
        to_int(re.search(pattern, line)[1])*10
        + to_int(re.search(".*" + pattern, line)[1])
...
# digits are actually spelled out with letters: one, two, three, four,
...
# --------------------
#
# A trickier puzzle than it might seem at first because the input
# contains character sequences such as "oneight" which preclude using
# the approach of identifying all non-overlapping digits from left to
# right.
...
def to_int(match):
    ...
    return mapping[match] if match in mapping else int(match)
