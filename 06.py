...
# wall near the ferry dock.  "Boat races!  Open to the public!  Grand
# prize is an all-expenses-paid trip to Desert Island!"  That must be
# where the sand comes from!  Best of all, the boat races are starting
# in just a few minutes.
#
# You manage to sign up as a competitor in the boat races just in
# time.  The organizer explains that it's not really a traditional
# race - instead, you will get a fixed amount of time during which
# your boat has to travel as far as it can, and you win if your boat
# goes the farthest.
#
...
# held.  The boats are much smaller than you expected - they're
# actually toy boats, each with a big button on top.  Holding down the
# button charges the boat, and releasing the button allows the boat to
# move.  Boats move faster if their button was held longer, but time
# spent holding the button counts against the total race time.  You
# can only hold the button at the start of the race, and boats don't
...
# - The second race lasts 15 milliseconds.  The record distance in
#   this race is 40 millimeters.
...
#   the boat will travel at a speed of 1 millimeter per millisecond
#   for 6 milliseconds, reaching a total distance traveled of 6
#   millimeters.
...
#   the race.  You never let go of the button.  The boat can't move
#   until you let go of the button.  Please make sure you let go of
...
with open("06.in") as f:
    ...
    best_distances = map(int, re.findall(r"\d+", f.readline()))

def num_ways(race_time, best_distance):
    return sum(
        d > best_distance
        for d in [(race_time-t)*t for t in range(1, race_time)]
    )

print(prod(num_ways(t, bd) for t, bd in zip(times, best_distances)))

# --- Part Two ---
#
# As the race is about to start, you realize the piece of paper with
# race times and record distances you got earlier actually just has
# very bad kerning.  There's really only one race - ignore the spaces
...
# and the record distance you need to beat is 940200 millimeters.  You
...
# --------------------
#
# It takes only 4 seconds to compute the solution using the above
# code, but it's easy to solve this analytically as it's just a
# quadratic equation (an inverted parabola) and we're looking for the
# number of integer solutions between the two roots.
...
# Equation is (T-t)*t - D = 0.

roots = [(-T + sqrt(T*T-4*D))/(-2), (-T - sqrt(T*T-4*D))/(-2)]
roots.sort()
