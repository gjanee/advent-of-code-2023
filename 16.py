...
# with cave, and the doorways are just cave, and the floor is cave,
...
# see a bright light in a cavern up ahead.  There, you discover that
# the beam of light you so carefully focused is emerging from the
# cavern wall closest to the facility and pouring all of its energy
...
#
# The contraption is aligned so that most of the beam bounces around
# the grid, but each tile on the grid converts some of the beam's
# light into heat to melt the rock in the cavern.
#
...
# passing through it at the same time.  A tile is energized if that
# tile has at least one beam pass through it, reflect in it, or split
# in it.
#
...
# of the beams.  If a tile contains beams moving in multiple
# directions, the number of distinct directions is shown instead.
# Here is the same diagram but instead only showing whether a tile is
# energized (#) or not (.):
#
# ######....
# .#...#....
...
grid = [line.strip() for line in open("16.in")]
R, C = len(grid), len(grid[0])  # grid dimensions

dirs = [(-1, 0), (0, 1), (1, 0), (0, -1)]  # directions in right turn order
N, E, S, W = range(4)  # direction indices

Rule = namedtuple("Rule", "symbol dirs turn splits")
...
def propagate(start):
    # Propagate a beam starting from a tile just outside the grid.
    # Return the number of tiles visited.
    #
    # We follow the beam until it ends either by leaving the grid,
    # arriving at a tile+direction already seen, or splitting.  The
    # beams resulting from splitting are in effect new beams, so we
    # maintain a todo list of beams to follow.
    todo = [start]
    seen = set()
    while len(todo) > 0:
        r, c, d = todo.pop()
        while True:
            ...
            if r not in range(R) or c not in range(C) or (r, c, d) in seen:
                break
            seen.add((r, c, d))
            rule = next(
                filter(
                    lambda rule: grid[r][c] == rule.symbol and d in rule.dirs,
                    rules
                )
            )
            if rule.turn != None:
                d = (d+rule.turn)%len(dirs)
            else:
                for nd in rule.splits:
                    todo.append((r, c, nd))
                break
    return len(set((r, c) for r, c, d in seen))

print(propagate((0, -1, E)))

# --- Part Two ---
#
# As you try to work out what might be wrong, the reindeer tugs on
# your shirt and leads you to a nearby control panel.  There, a
# collection of buttons lets you align the contraption so that the
# beam enters from any edge tile and heading away from that edge.
# (You can choose either of two directions for the beam if it starts
# on a corner; for instance, if the beam starts in the bottom-right
# corner, it can start heading either left or upward.)
#
# So, the beam could start on any tile in the top row (heading
# downward), any tile in the bottom row (heading upward), any tile in
# the leftmost column (heading right), or any tile in the rightmost
# column (heading left).  To produce lava, you need to find the
# configuration that energizes as many tiles as possible.
#
# In the above example, this can be achieved by starting the beam in
# the fourth tile from the left in the top row:
#
# .|<2<\....
# |v-v\^....
# .v.v.|->>>
...
print(
    max(
        max(propagate(( r, -1, E)) for r in range(R)),
        max(propagate(( r,  C, W)) for r in range(R)),
        max(propagate((-1,  c, S)) for c in range(C)),
        max(propagate(( R,  c, N)) for c in range(C))
    )
)
