...
# disappears before your eyes!  To be fair, she had just finished
# warning you about ghosts a few minutes ago.
#
# One of the camel's pouches is labeled "maps" - sure enough, it's
# full of documents (your puzzle input) about how to navigate the
# desert.  At least, you're pretty sure that's what they are; one of
# the documents contains a list of left/right instructions, and the
# rest of the documents seem to describe some kind of network of
# labeled nodes.
#
...
# next left/right instruction in your input.  In this example, start
# with AAA and go right (R) by choosing the right element of AAA, CCC.
...
# left/right instructions, repeat the whole sequence of instructions
# as necessary: RL really means RLRLRLRLRLRLRLRL... and so on.  For
...
class Node(namedtuple("Node_base", "label i")):
    """A Node encompasses a label and where we are in the
    instruction list."""

    def next(self):
        return Node(
            graph[self.label][0 if instructions[self.i] == "L" else 1],
            (self.i+1)%len(instructions)
        )
...
# barely left your starting position.  It's going to take
# significantly more steps to escape!
#
# What if the map isn't for people - what if the map is for ghosts?
# Are ghosts even bound by the laws of spacetime?  Only one way to
# find out.
#
...
# --------------------
#
# The paths each follow a "prefix" section before entering into a
# cycle.  It becomes quickly apparent that we can't simply follow the
# paths explicitly, but must analyze the cycles.  This could
# potentially be a difficult or at least messy puzzle, but fortunately
# the regular nature of the input simplifies things greatly.
#
# The length of the instruction list is prime, so the cycles all have
# lengths a multiple of that prime.  Each path encounters exactly one
# terminal node (i.e, node ending in Z) which occurs within the cycle
# section of the path.  Furthermore, if the length of a path's prefix
# section is P, then the terminal node occurs exactly P steps before
# the path cycles back.  An illustration:
#
#       +----+
#       |    |
#       Z    |  ^
...
# <- P ->
#
# As a consequence, if a path's cycle length is N, then it encounters
# a terminal node exactly every N steps.  Thus all paths first
# simultaneously land on their terminal nodes at the least common
# multiple of their cycle lengths.
