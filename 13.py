...
# which launches you neatly and precisely up to the edge of Lava
# Island.
#
# There's just one problem: you don't see any lava.
#
# You do see a lot of ash and igneous rock; there are even what look
# like gray mountains scattered around.  After a while, you make your
# way to a nearby cluster of mountains only to discover that the
# valley between them is completely full of large mirrors.  Most of
# the mirrors seem to be aligned in a consistent way; perhaps you
...
# reflection across either a horizontal line between two rows or
# across a vertical line between two columns.
#
# In the first pattern, the reflection is across a vertical line
# between two columns; arrows on each of the two columns point at the
...
# 7 #....#..# 7
#
# This pattern reflects across the horizontal line between rows 4 and
# 5.  Row 1 would reflect with a hypothetical row 8, but since that's
...
# has 5 columns to its left and the second pattern's horizontal line
...
grids = []
for s in open("13.in").read().split("\n\n"):
    g = s.split()
    gT = [
        "".join(g[r][c] for r in range(len(g)))
        for c in range(len(g[0]))
    ]
    grids.append((g, gT))

def hamming_distances(grid):
    # Return a list of the Hamming distances that result from
    # reflecting the grid between each pair of rows, beginning with a
    # line of reflection between rows 0 and 1.  A Hamming distance of
    # 0 indicates a perfect reflection.
    return [
        sum(
            grid[r-1-k][c] != grid[r+k][c]
            for k in range(min(len(grid)-r, r))
            for c in range(len(grid[0]))
        )
        for r in range(1, len(grid))
    ]

def solve(target_distance):
    n = 0
    for g, gT in grids:
        hds = hamming_distances(g)
        if target_distance in hds:
            n += 100*(hds.index(target_distance)+1)
        else:
            hds = hamming_distances(gT)
            n += hds.index(target_distance)+1
    return n
...
#
# In each pattern, you'll need to locate and fix the smudge that
# causes a different reflection line to be valid.  (The old reflection
# line won't necessarily continue being valid after the smudge is
# fixed.)
#
...
# reflection lines.  In this example, the first pattern's new
# horizontal line has 3 rows above it and the second pattern's new
